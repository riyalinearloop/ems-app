...
interface Step3RefillWitnessSignatureProps {
  omitWitness: boolean;
  witnessName?: string;
...
const Step3RefillWitnessSignature = ({
  omitWitness,
  witnessName = "",
...
}: Step3RefillWitnessSignatureProps) => {
  return (
      ...
      <div className="bg-white rounded-lg shadow-md p-4 sm:p-5 md:p-6">
            ...
            <div>
              ...
              <div className="space-y-2 sm:space-y-3">
                <input
                  ...
                  placeholder="Enter supervisor name or scan QR code"
        ...
        <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-gray-50 rounded-lg">
          ...
          <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm">
            ...
            <p>
              <span className="font-medium">Refilled by:</span>{" "}
              {witnessName || "N/A"}
            </p>
            <p>
            ...
            {selectedPouch && (
              <div className="mt-3">
                <p className="font-medium">Refill Details:</p>
                {selectedPouch.medications.map((med, index) => {
                  const current = med.quantity;
                  const refill = refillQuantities[med.name] || 0;
                  const newTotal = current + refill;
                  if (refill > 0) {
                    return (
                      <p key={index} className="text-green-600 ml-2">
                        {med.name} {med.dosage || ""}: {current} → {newTotal} (+
                        {refill})
                      </p>
                    );
                  }
...
export default Step3RefillWitnessSignature;
